class HablaYaApp {
    ...
    async handleUserMessage(message) {
        ...
        try {
            ...
            // Update session stats with level detection
            if (aiResponse.metadata.detectedLevel) {
                this.sessionStats.detectedLevel = aiResponse.metadata.detectedLevel;
                this.sessionStats.cefrLevel = aiResponse.metadata.cefrLevel;
            }
    ...
    async getAIResponse(userMessage) {
        ...
        try {
            ...
            // Update user level if auto-detected
            if (data.metadata.detectedLevel && data.metadata.detectedLevel !== this.userSettings.level) {
                this.updateUserLevel(data.metadata.detectedLevel, data.metadata.cefrLevel);
            }
    ...
    addMessage(sender, content, isVoiceInput = false, metadata = {}) {
        ...
        // Add speaking animation class for AI messages
        if (sender === 'ai') {
        ...
        const avatarIcon = sender === 'user' ? 
            ...
            `<svg viewBox="0 0 24 24" width="16" height="16"><path fill="currentColor" d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>`;
        const senderName = sender === 'user' ? 'You' : 'HablaYa! AI Tutor';
        
        // Add level indicator for AI messages if level was detected
        const levelIndicator = metadata.detectedLevel ? 
            `<span class="level-indicator level-${metadata.detectedLevel.replace('-', '-')}">${metadata.cefrLevel}</span>` : '';
        ...
        // Remove speaking animation after a delay
        if (sender === 'ai') {
            setTimeout(() => {
    ...
    addPronunciationFeedback(analysis) {
        if (analysis.suggestions && analysis.suggestions.length > 0) {
            ...
            // Create voice feedback message with special styling
            const messageElement = document.createElement('div');
            messageElement.className = 'message voice-feedback';
            messageElement.innerHTML = `
                <div class="message-content">
                    <div class="message-text">
    ...
    showTypingIndicator() {
        ...
        typingElement.innerHTML = `
            <div class="message-avatar">
                <svg class="avatar-icon" viewBox="0 0 24 24" width="16" height="16">
                    <path fill="currentColor" d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                </svg>
            </div>
            <div class="message-content">
    ...
    updateSessionStats(text) {
        ...
        if (this.sessionStats.detectedLevel) {
            this.sessionStats.cefrLevel = this.getCEFRCode(this.sessionStats.detectedLevel);
        }
    ...
    updateUserLevel(detectedLevel, cefrLevel) {
        const oldLevel = this.userSettings.level;
        this.userSettings.level = detectedLevel;
        
        // Update UI
        this.userLevel.value = detectedLevel;
        
        // Show level update notification
        this.addSystemMessage(`Level detected: ${detectedLevel} (${cefrLevel}) - I've adjusted my responses to match your level!`);
        
        // Save to localStorage
        localStorage.setItem('hablaya-settings', JSON.stringify(this.userSettings));
        
        // Add level indicator to progress
        this.addLevelIndicator(detectedLevel, cefrLevel);
    }
    
    addLevelIndicator(level, cefrLevel) {
        const levelElement = document.createElement('div');
        levelElement.className = `level-indicator level-${level.replace('-', '-')}`;
        levelElement.textContent = `${level} (${cefrLevel})`;
        
        // Add to header stats temporarily
        const statsContainer = document.querySelector('.learning-stats');
        if (statsContainer) {
            statsContainer.appendChild(levelElement);
